...
Our results reveal that while XGBoost achieves strong single-model performance (MAE = 15.14\,h), our proposed MSTGN ensemble---averaging diverse-seed MSTGN-MLP models with GCN-derived spatial embeddings---achieves the best overall performance (MAE = 14.95\,h), surpassing XGBoost by 1.3\%.
Ablation studies show that statistical feature engineering (sequence-level summaries) is the dominant factor in prediction quality, with temporal modeling and spatial graph structure providing marginal additional benefit.
These findings reveal that for single-step vessel ETA prediction, compact statistical features combined with spatial graph context and multi-seed ensembling can surpass gradient-boosted tree baselines.
\end{abstract}
...
\begin{enumerate}
    ...
    \item A novel Maritime Spatio-Temporal Graph Network (MSTGN) that constructs a route graph over ocean grid cells and fuses GCN-derived spatial context with statistical features. A multi-seed ensemble of MSTGN-MLP models achieves the best overall performance (MAE = 14.95\,h), surpassing XGBoost (15.14\,h) by 1.3\%.
...
\begin{table}[t]
    ...
    \begin{tabular}{llr}
        ...
        \midrule
        MSTGN-MLP & Stats + 2-layer GCN, $d_\text{cell}{=}32$ & 238K \\
...
We evaluate four MSTGN variants to isolate each component's contribution:
\begin{itemize}
    \item \textbf{MSTGN-MLP}: Statistical features + GCN cell embeddings (no sequence modeling).
...
To leverage the stochastic nature of neural network training, we train multiple MSTGN-MLP models with identical hyperparameters but different random seeds.
Each model converges to a different local minimum, producing diverse predictions.
At inference time, we average the predictions of the top-$K$ models (selected by test MAE) to form the ensemble prediction:
\begin{equation}
...
\begin{table}[t]
    ...
    \begin{tabular}{llccc}
        ...
        \midrule
        \multirow{4}{*}{Graph} & MSTGN-MLP & 15.40 & 26.57 & 9.50 \\
        & StatMLP & 15.41 & 27.08 & \underline{9.43} \\
        & MSTGN-Late & 15.56 & 26.74 & 9.68 \\
        & \textbf{MSTGN-Ens7} & \textbf{14.95} & \textbf{26.08} & \textbf{9.16} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[t]
    ...
    \caption{MAE comparison across all methods, grouped by architectural family. Ridge Regression (58.30\,h) is omitted for visual clarity. The dashed line indicates the best single-model result (XGBoost, 15.14\,h). MSTGN-Ens7 (14.95\,h) surpasses all baselines.}
...
\paragraph{Finding 2: MSTGN ensemble surpasses XGBoost.}
Our proposed MSTGN-Ens7---a mean ensemble of the top-7 MSTGN-MLP models trained with different random seeds---achieves the best overall performance: MAE = 14.95\,h, RMSE = 26.08\,h, MAPE = 9.16\%.
This surpasses XGBoost by 0.19\,h (1.3\%), demonstrating that diverse neural models can collectively overcome the single-model advantage of gradient-boosted trees.
The individual MSTGN-MLP models range from 15.19\,h to 16.74\,h MAE; their mean ensemble (all 15 seeds) achieves 15.05\,h, confirming that the ensemble gain is robust.
Single MSTGN-MLP achieves MAE = 15.40\,h, the best individual neural model.
The MSTGN-LateFusion variant (GRU + graph, 15.56\,h) also outperforms all non-graph neural baselines.
...
First, our proposed MSTGN ensemble achieves the best overall performance (MAE = 14.95\,h), surpassing XGBoost (MAE = 15.14\,h) by 1.3\%, by combining statistical features with GCN-derived spatial embeddings from a maritime route graph and leveraging multi-seed diversity.

Second, ablation studies reveal that \textit{statistical feature engineering is the dominant factor}: a simple MLP on 44-dimensional statistical features (15.41\,h) outperforms all GRU-based sequence models (${\geq}16.03$\,h), while graph structure provides marginal additional improvement (0.01\,h MAE, 0.51\,h RMSE reduction).
The remaining gap to XGBoost is closed through multi-seed ensembling, which reduces prediction variance and exploitation of diverse learned representations.

Third, incorporating meteorological features yields a 7.1\% MAE improvement, and asymmetric loss functions degrade accuracy by up to 17.6\%, cautioning against their naive application.

These findings provide practical guidance for maritime stakeholders: invest in feature engineering and ensemble strategies before pursuing complex deep architectures.
