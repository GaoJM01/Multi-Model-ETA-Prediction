...
Taken together, the studies reviewed above show that maritime ETA prediction has advanced through both explicit route modeling and increasingly strong feature-engineering pipelines~\cite{parkVesselEstimatedTime2021,yoonEnhancingContainerVessel2023,yuShipArrivalPrediction2018,chuEvaluationPredictionPunctuality2023,kolleyRobustBerthScheduling2022,leiPredictingVesselArrival2024,chuVesselArrivalTime2025,saberHighaccuracyPredictionVessels2025,duSegmentedETAPrediction2025}. However, the existing evidence remains fragmented: many studies are developed for port areas, inland waterways, pilotage zones, or specific multi-port settings, while even cross-Pacific evaluation has often focused on a near-arrival regime rather than the full remaining voyage~\cite{chuEvaluationPredictionPunctuality2023,kolleyRobustBerthScheduling2022,leiPredictingVesselArrival2024,duSegmentedETAPrediction2025,chuVesselArrivalTime2025,lloretbatlle2025}. As a result, it is still difficult to determine whether reported gains from recurrent, convolutional, and other deep sequence models reflect genuine architectural advantages for maritime ETA prediction, or instead arise from differences in route scope, feature construction, and evaluation design~\cite{elmekkaouiDeepLearningModels2022,wenzelNeuralNetworkApproach2023,duSegmentedETAPrediction2025}. A related unresolved issue is whether, for remaining-time prediction over long open-ocean voyages, explicit route information provides substantial predictive value beyond compact feature-based summaries of vessel dynamics and auxiliary covariates~\cite{parkVesselEstimatedTime2021,yoonEnhancingContainerVessel2023,leiPredictingVesselArrival2024,chuVesselArrivalTime2025}. These limitations motivate the present study to examine full-voyage ETA prediction on trans-Pacific routes under a unified experimental protocol and thereby clarify which modeling ingredients are genuinely beneficial for this task.
...
\begin{itemize}
    ...
    \item \textbf{MedAE} (Median Absolute Error, in hours): the median of $|\hat{y}_i - y_i|$, which is robust to outliers and reflects typical prediction quality.
    \item $\boldsymbol{R^2}$ (Coefficient of Determination): $1 - \sum_i(\hat{y}_i - y_i)^2 / \sum_i(y_i - \bar{y})^2$.
    \item \textbf{Within-$\tau$} accuracy: the fraction of predictions with $|\hat{y}_i - y_i| \leq \tau$ hours, reported for $\tau \in \{6, 12, 24\}$.
\end{itemize}

For uncertainty quantification (Section~\ref{sec:uq}), we additionally report:
\begin{itemize}
    \item \textbf{PICP} (Prediction Interval Coverage Probability): the fraction of test samples whose true value falls within the predicted interval.
    \item \textbf{MPIW} (Mean Prediction Interval Width, in hours): the average width of prediction intervals, measuring sharpness.
    \item \textbf{DSCE} (Duration-Stratified Calibration Error): the maximum absolute deviation between the stratum-level PICP and the target coverage across voyage duration bins, measuring worst-case miscalibration.
\end{itemize}
...
Table~\ref{tab:extended_metrics} supplements the main results with additional metrics for the key models.
MSTGN-KD achieves MedAE~$=$~8.31\,h (indicating that the typical prediction is within $\pm$8\,h), $R^2 = 0.958$, and 41.1\% of predictions within $\pm$6\,h of the true value.
The ensemble further improves MedAE to 8.09\,h and within-24h accuracy to 82.1\%.

\begin{table}[t]
    \centering
    \caption{Extended evaluation metrics for key models. Within-$\tau$: fraction of predictions with $|\text{error}| \leq \tau$\,h.}
    \label{tab:extended_metrics}
    \footnotesize
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model} & \textbf{MedAE} & $\boldsymbol{R^2}$ & \textbf{$\pm$6\,h} & \textbf{$\pm$12\,h} & \textbf{$\pm$24\,h} \\
        \midrule
        MSTGN-MLP2   & 8.13 & 0.957 & 41.7\% & 61.6\% & 81.5\% \\
        MSTGN-KD     & 8.31 & 0.958 & 41.1\% & 61.0\% & 81.6\% \\
        MSTGN-Ens7   & \textbf{8.09} & \textbf{0.959} & \textbf{41.5\%} & \textbf{61.5\%} & \textbf{82.1\%} \\
        \bottomrule
    \end{tabular}
\end{table}
...
The $2^\circ{\times}2^\circ$ resolution achieves the best MAE (15.28\,h) and MAPE (9.33\%), confirming our empirical choice.
At $1^\circ$ resolution, the graph contains $3{\times}$ more nodes (4,665 vs.\ 1,541) and yields the lowest RMSE (26.24\,h), suggesting that finer spatial granularity helps reduce extreme-error outliers.
However, the sparser graph (many cells fail the minimum-50-count threshold) leads to higher MAE (+0.23\,h) and MAPE (+0.57\,pp), indicating that insufficient per-cell training data degrades average prediction quality.
At $4^\circ$ resolution, each node aggregates $\sim\!440$\,km of ocean, conflating topologically diverse corridors into a single embedding; all three metrics are worst among the tested resolutions.
The $2^\circ$ grid thus balances spatial detail against data sufficiency for learning meaningful cell embeddings.
...
we extract the learned GCN cell embeddings from the trained MSTGN-MLP2 checkpoint, then train a new XGBoost model using the same 81-dimensional statistical feature set plus the 96-dimensional GCN context features (current cell, origin cell, route mean embedding), yielding a 177-dimensional feature vector.
...
The results reveal three key insights.
\textbf{(1)~Richer statistical features are the primary driver.}
Expanding the feature set from 44 to 81 dimensions improves XGBoost from 15.14 to 14.97\,h ($-$0.17\,h), confirming that the speed, position, and course aggregations introduced in our pipeline are highly informative.
\textbf{(2)~GCN embeddings do not help tree models.}
Appending 96-dim graph context to XGBoost slightly \emph{hurts} MAE (+0.05\,h), indicating that the spatial information captured by the GCN is redundant with the statistical features when consumed by a gradient-boosted ensemble.
\textbf{(3)~The neural head trades MAE for RMSE, and KD closes the gap.}
MSTGN-MLP2 (no KD) has higher MAE than XGBoost (15.28 vs.\ 14.97\,h) because the shallow MLP head is a weaker function approximator for tabular data.
However, knowledge distillation from the 7-model ensemble reduces MSTGN-MLP2's MAE to 15.13\,h ($-$0.15\,h) and produces the soft-target signal that enables the final ensemble (14.95\,h) to surpass XGBoost.
These findings affirm that MSTGN-MLP2 is best understood as a \emph{graph-augmented feature fusion framework} whose primary value lies in (a)~the engineered statistical features and (b)~ensemble diversity, rather than in superior single-model capacity over tree methods.
...
This MAE--reliability trade-off is a fundamental consequence of the skewed voyage duration distribution and the log-space training objective, and motivates the uncertainty quantification framework presented next.

\subsection{Uncertainty Quantification via Conformal Prediction}
\label{sec:uq}

Port operators require not only a point ETA estimate but also a measure of \textit{how uncertain} that estimate is, enabling dynamic buffer allocation and risk-aware berth scheduling.
We leverage the multi-seed ensemble's prediction disagreement as an epistemic uncertainty signal and apply \textit{split conformal prediction}~\cite{vovk2005conformal,romano2019cqr} to produce calibrated prediction intervals with finite-sample coverage guarantees.
...
Both methods achieve the target coverage, confirming conformal validity.
The adaptive intervals are 8--9\% narrower than fixed-width intervals (64.3 vs.\ 69.8\,h at 90\%), demonstrating that ensemble uncertainty is a meaningful signal: the model ``knows when it doesn't know.''
The Spearman rank correlation between $\hat{\sigma}$ and absolute error is $\rho = 0.568$ ($p < 10^{-300}$), confirming strong monotonic alignment between predicted uncertainty and actual error magnitude.

\subsubsection{Stratified Calibration Analysis}

While marginal coverage is near-exact, calibration may degrade for specific subpopulations.
We propose \textbf{Duration-Stratified Calibration Error (DSCE)}, defined as the maximum absolute deviation between stratum-level PICP and the target coverage across voyage duration bins:
\begin{equation}
    \text{DSCE} = \max_{s \in \mathcal{S}} \bigl|\text{PICP}_s - (1{-}\alpha)\bigr|,
\end{equation}
where $\mathcal{S}$ partitions samples by true remaining time.
Lower DSCE indicates more uniform reliability across voyage stages.

Table~\ref{tab:stratified_calib} reports 90\% adaptive interval metrics stratified by voyage duration.

\begin{table}[t]
    \centering
    \caption{Stratified calibration of 90\% adaptive conformal intervals by voyage duration. RPIW $=$ MPIW / median\_true\_duration.}
...
Short voyages ($< 100$\,h) are \textit{over-covered} (94.8\% vs.\ 90\% target), meaning intervals are conservative---acceptable for operational safety.
Long voyages ($> 400$\,h) are \textit{under-covered} (85.6\%), reflecting the same long-tail difficulty identified in Section~\ref{sec:large_dev}.
The DSCE of 4.78\,pp indicates that worst-case stratum-level miscoverage is moderate; no stratum deviates by more than 5 percentage points from the target.

\subsubsection{Operational Buffer Time Analysis}

For port operators, the prediction interval can be translated into a \textit{buffer time}---the additional slack to add to the point ETA to achieve a desired on-time reliability.
Table~\ref{tab:buffer} reports the empirical buffer needed for 90\% and 95\% on-time arrival, stratified by remaining voyage duration.
...
For near-arrival predictions ($< 1$ day remaining), a buffer of just 1.5\,h suffices for 90\% reliability---directly usable for berth scheduling.
For early-voyage predictions ($> 14$ days), the 90\% buffer grows to 67.8\,h ($\sim$3 days), reflecting the fundamental uncertainty of long-horizon maritime forecasting.
The monotonically increasing late-arrival rate (30.4\% to 62.4\%) confirms the systematic underestimation bias in the tail identified in the large-deviation analysis.
These duration-specific buffers enable \textit{risk-aware scheduling}: port operators can apply tighter buffers for near-arrival vessels (high confidence) and wider buffers for distant vessels (high uncertainty), optimizing berth utilization while maintaining service reliability.
