...
\begin{table}[t]
    \centering
    \caption{Ablation: Isolating Graph Information from Neural Fusion Head. ``Stats'' refers to 81-dim statistical features (same aggregations as MSTGN-MLP2); ``GCN'' refers to 96-dim GCN cell embeddings (current + origin + route mean).}
    ...
    \begin{tabular}{lccccc}
        ...
        XGBoost (Stats-only)      & 81-d  &             &             & TBD & TBD \\
        XGBoost (Stats + GCN)     & 81-d  & \checkmark  &             & TBD & TBD \\
        MSTGN-MLP2 (no KD)        & 81-d  & \checkmark  & \checkmark  & 15.28 & 26.58 \\
        MSTGN-MLP2 + KD           & 81-d  & \checkmark  & \checkmark  & \textbf{15.13} & \textbf{26.16} \\
        \bottomrule
    \end{tabular}
\end{table}
